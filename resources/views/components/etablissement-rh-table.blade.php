@props(['etablissement_rhs', 'action_charter'])
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <span class="text-muted small">
            <button class="btn btn-primary quick-btn">
                <a href="{{ route('etablissement_rh.createByCharter', $action_charter) }}" class="text-white text-decoration-none">
            ...
            <table class="table table-bordered table-hover text-center align-middle" style="direction: rtl;">
                <thead class="table-light">
                    <tr>
                        ...
                        <th rowspan="2">{{-- afficher value filter-select --}}
                ...
                <tbody>
                    ...
                    @foreach($etablissement_rhs as $etablissement)
                        @php
                        ...
                        <tr data-is-public="{{ $etablissement->is_public ? 'public' : 'private' }}">
                            ...
                            <td>{{ $etablissement->is_public ? $etablissement->grade : $etablissement->specialisation }}</td>
                            <td>{{ $males }}</td>
                            <td>{{ $females }}</td>
                            <td><strong>{{ $sum }}</strong></td>
                            <td>
                                <a href="{{ route('etablissement_rh.edit', $etablissement->id) }}" class="btn btn-sm btn-outline-primary me-1">
                                    <i class="fa-solid fa-pen"></i>
                                </a>
                                <form action="{{ route('etablissement_rh.destroy', $etablissement->id) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من الحذف؟')">
                                    @csrf
                                    @method('DELETE')
