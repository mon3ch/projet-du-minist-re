    ...
    <div class="content">
        @include('partials.sidebar.admin.sidebar')
                <div class="card-header  d-flex justify-content-between align-items-center">
                    <h4 class="mb-0"><i class="fa-solid fa-users-gear me-2"></i>  {{ __('messages.add_nature_projet') }}</h4>
                </div>
        <div class="container mt-4">
   <div class="soft-card p-3 mb-4" data-aos="fade-up">

                <div class="card-body p-4">
                    <form action="{{ route('type_projet.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label for="name" class="form-label">{{ __('messages.type_projet_name') }}</label>
                                <input type="text" class="form-control" id="name" name="name" value="{{ old('name') }}" placeholder="{{ __('messages.type_projet_name_placeholder') }}">
                                @error('name')
                                ...
                                <select name="nature_projet_id" id="nature_projet_id" 
                                    ...
                                    @foreach($nature_projets as $nature_projet)
                                        <option value="{{ $nature_projet->id }}" 
                                            {{ old('nature_projet_id') == $nature_projet->id ? 'selected' : '' }}>
                                            {{ $nature_projet->name }}
                                        </option>
                                    @endforeach
                                </select>
